...
use_FACS <- subset(FACS, select = -c(user_login, user_email, country, training_id, timespan))

use_SETT <- subset(SETT, select = -c(user_login, user_email, country, training_id, timespan))

use_METT <- subset(METT, select = -c(user_login, user_email, country, training_id, timespan))

use_RETT_F <- subset(RETT_F, select = -c(user_login, user_email, country, training_id, timespan))

use_RETT_LE <- subset(RETT_LE, select = -c(user_login, user_email, country, training_id, timespan))

use_RETT_W <- subset(RETT_W, select = -c(user_login, user_email, country, training_id, timespan))
```
...
```{r SETT, fig.height = 20, fig.width = 22, echo = FALSE}
# This looks at the trend of training tool score
...
```{r METT, fig.height = 20, fig.width = 22, echo = FALSE}
# This looks at the trend of training tool score
...
## Training History of Tools: RETT Family
```{r RETT_F, fig.height = 20, fig.width = 22, echo = FALSE}
# This looks at the trend of training tool score
# by customer for RETT Family
RETT_F_ID <- use_RETT_F %>%
  group_by(user_id) %>%
  unique()
...
```{r RETT_F-number-of-times-taken, echo = FALSE}
# This looks at the number of times RETT Family is taken
# on average by customers
RETT_F_ID %>%
  group_by(user_id) %>%
  tally() %>%
...
## Training History of Tools: RETT Workplace
```{r RETT_W, fig.height = 20, fig.width = 22, echo = FALSE}
# This looks at the trend of training tool score
# by customer for RETT Workplace
RETT_W_ID <- use_RETT_W %>%
  group_by(user_id) %>%
  unique()
...
```{r RETT_W-number-of-times-taken, echo = FALSE}
# This looks at the number of times RETT Workplace is taken
# on average by customers
RETT_W_ID %>%
  group_by(user_id) %>%
  tally() %>%
...
## Training History of Tools: RETT Law Enforcement
```{r RETT_LE, fig.height = 20, fig.width = 22, echo = FALSE}
# This looks at the trend of training tool score
# by customer for RETT Law Enforcement
RETT_LE_ID <- use_RETT_LE %>%
  group_by(user_id) %>%
  unique()
...
```{r RETT_LE-number-of-times-taken, echo = FALSE}
# This looks at the number of times RETT Law Enforcement is taken
# on average by customers
RETT_LE_ID %>%
  group_by(user_id) %>%
  tally() %>%
