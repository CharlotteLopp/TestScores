...
```{r our-customer-base-by-occupation-by-sex, echo = FALSE}
total_tools %>%
  filter(Sex != "") %>%
  filter(Occupation != "") %>%
...
```{r our-customer-base-by-sex-by-occupation, echo = FALSE}
total_tools %>%
  filter(Sex != "") %>%
  filter(Occupation != "") %>%
  group_by(Occupation, Sex) %>%
...
```{r redo-table-of-referrals, echo = FALSE}
...
```{r top-performers-in-METT-and-SETT, echo = FALSE}
total_tools %>%
  ...
  ylab("Number of Users")
```
**Note:** This shows our customer base in terms of top performers by occupation divided by METT vs SETT.
